...
export default class RoleEdit extends Component<{match: any}> {
    ...
    componentDidMount = async () => {
        ...
        const rolesCall = await axios.get(`roles/${this.id}`);
                            ...
                            {this.state.permissions.map(
                                (p: Permission) =>{
                                    return (
                                        ...
                                        <input className="form-check-input" type="checkbox" value={p.id} defaultChecked={this.isChecked(p.id)} onChange={e=>this.check(p.id)}/>
